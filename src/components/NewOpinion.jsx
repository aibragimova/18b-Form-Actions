...
export function NewOpinion() {
  ...
  async function shereOpinionAction(prevState, formData) {
    ...
    if (title.trim().length > 5) {
      errors.push('Title must be at least five charecters long.')
    }
    if (body.trim().length < 10 || body.trim().length > 300) {
      errors.push('Opinion must be letween 10 and 300 characters long.')
    }
